...
public class TestforCityHash {
	...
	public static void main(String args[]) throws Exception {
		...
		List<chunk> aaa=ChunkProcess.GetChunk(path,0,8,0,chunkType.FIX);
		System.out.println(aaa);
		List<chunk> bbb=ChunkProcess.GetChunk(path,0,16,0,chunkType.VAR);
		System.out.println(bbb);
		List<chunk> ccc=ChunkProcess.GetChunk(path,0,64,0,chunkType.NO);
		System.out.println(ccc);		
		System.out.println(clsJavaVariableChunk.GetVariableChunks(path,0,1,32));		
		System.out.println(clsJavaVariableChunk.GetVariableChunks(path,0,2,8));
		return;
